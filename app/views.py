...
class ChartData(APIView):
    ...
    def get(self, request, format=None):
        ...
        global_pie_percent_of_wins = [round(1.0 * wins / games, 2), round(1.0 * loses / games, 2),
                                      round(1.0 * draws / games, 2)]

        choices = global_stat.rocks + global_stat.scissors + global_stat.papers
        global_polar_story_choices = [round(1.0 * global_stat.rocks / choices, 2),
                                      round(1.0 * global_stat.scissors / choices, 2),
                                      round(1.0 * global_stat.papers / choices, 2)]

        global_bubble_story = []
        with connection.cursor() as cursor:
            for i in range(1, 11):
                cursor.execute(
                    "SELECT (SUM(wins) - SUM(loses)) as result FROM app_playerui WHERE wins+draws+loses < %s AND wins+draws+loses >= %s",
                    [i * 10, (i - 1) * 10])
                summary = cursor.fetchone()[0]
                cursor.execute(
                    "SELECT COUNT(*) as result FROM app_playerui WHERE wins+draws+loses < %s AND wins+draws+loses >= %s",
                    [i * 10, (i - 1) * 10])
        ...
        if games == 0:
            games = 1
        local_pie_percent_of_wins = [round(1.0 * wins / games, 2), round(1.0 * loses / games, 2),
                                     round(1.0 * draws / games, 2)]
        ...
        if choices == 0:
            choices = 1
        local_polar_story_choices = [round(1.0 * str_of_choices.count('r') / choices, 2),
                                     round(1.0 * str_of_choices.count('s') / choices, 2),
                                     round(1.0 * str_of_choices.count('p') / choices, 2)]
